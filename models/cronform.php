 ...
 * @since  1.6
 */
class Auto79ModelCronForm extends JModelForm
{
	private $item = null;
	...
	{
		// Get the form.
		$form = $this->loadForm('com_auto79.cron', 'cronform', array(
				'control'   => 'jform',
				'load_data' => $loadData
			)
		);
	...
	{
		$data = JFactory::getApplication()->getUserState('com_auto79.edit.cron.data', array());
		...
		$data->link = $array;
		// Support for multiple or not foreign key field: adcategories
		$array = array();

		foreach ((array) $data->adcategories as $value)
		{
			if (!is_array($value))
			{
				$array[] = $value;
	...
	{
		$id    = (!empty($data['id'])) ? $data['id'] : (int) $this->getState('cron.id');
