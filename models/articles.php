 ...
 * @since  1.6
 */
class Auto79ModelArticles extends JModelItem
{
	/**
	...
	public function &getData($id = null)
	{
		...
		if (isset($this->_item->category_id) && $this->_item->category_id != '')
		{
			...
			{
				$db    = JFactory::getDbo();
				$query = "SELECT  FROM  WHERE category_id LIKE '" . $value . "'";
			...
			$this->_item->category_id = !empty($textValue) ? implode(', ', $textValue) : $this->_item->category_id;
		}

		if (isset($this->_item->province) && $this->_item->province != '')
		{
			if (is_object($this->_item->province))
			{
				$this->_item->province = ArrayHelper::fromObject($this->_item->province);
			}

			$values = (is_array($this->_item->province)) ? $this->_item->province : explode(',',$this->_item->province);
			...
			$this->_item->province = !empty($textValue) ? implode(', ', $textValue) : $this->_item->province;
		}
